...
const News: React.FC<NewsProps> = ({ id }) => {
  ...
  const hasMore = NEWS_ITEMS.length > ITEMS_TO_SHOW;
          ...
          <div className="max-w-4xl mx-auto">
            <ul className="space-y-1">
              {NEWS_ITEMS.map((item, index) => {
                const shouldRender = showAll || isAnimating || index < ITEMS_TO_SHOW;
                if (!shouldRender) {
                  return null;
                }

                const animation =
                  showAll && !isAnimating && index >= ITEMS_TO_SHOW
                    ? 'slideDown 0.3s ease-out forwards'
                    : isAnimating && index >= ITEMS_TO_SHOW
                    ? 'slideUp 0.3s ease-out forwards'
                    : undefined;

                const animationDelay =
                  index >= ITEMS_TO_SHOW ? `${(index - ITEMS_TO_SHOW) * 0.1}s` : undefined;
